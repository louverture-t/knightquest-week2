Knights Quest - Product Requirements Document
Project Overview
FieldDetailsProject NameKnights QuestTypeCommand-Line Application (CLI)ModuleDatabase Foundations (Module 2)Duration1 WeekThemeKing Arthur / Knights of the Round Table
Summary
...
Tech Stack
LayerTechnologyVersionPurposeRuntimeNode.jsv18+JavaScript execution (ES Modules support)LanguageJavaScriptES6+Application logic (no TypeScript)Module SystemES Modules-import/export syntaxDatabasePostgreSQL14+Relational data storageDB Clientpg (node-postgres)8.xDatabase connection and queriesCLI Interfaceinquirer9.xInteractive command-line prompts (ES Modules)Configurationdotenv16.xEnvironment variable managementDB GUIDBeaverLatestDatabase visualization and testing
...
javascript// Transaction helper function
...
```
Test Case: Complete Quest Creation
──────────────────────────────────────────────────────

1. Launch: node src/cli.js
   Expected: Welcome message displays

2. Select Realm: Camelot
   Expected: Shows 5 Camelot characters

3. Select Characters: Sir Lancelot, Merlin
   Expected: Proceeds to item assignment

4. Assign Items:
   - Lancelot: Excalibur, Round Table Shield
   - Merlin: Merlin's Staff, Healing Potion
   Expected: All characters have ≥1 item

5. Enter Title: "The Quest for the Holy Grail"
   Expected: Proceeds to summary

6. Review Summary:
   Expected: Shows realm, characters, items, title

7. Confirm: Yes
   Expected: "Quest created with ID: X"

8. Create Another: No
   Expected: Goodbye message, clean exit

9. Verify in DBeaver:
   - SELECT * FROM quests WHERE id = X
   - SELECT * FROM quest_assignments WHERE quest_id = X
   Expected: 1 quest row, 4 assignment rows
Grading Alignment
Rubric ItemWeightWhat Demonstrates ItSchema design & relationships25%5 tables, proper FKs, data typesWorking CLI flow with inquirer30%All prompts work, validation, loopCorrect SQL queries and pg usage20%Parameterized queries, proper resultsInsert with transaction15%BEGIN/COMMIT/ROLLBACK patternREADME, seeds, polish10%Documentation, themed data, clean code